...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      className="liquid-glass mx-4 mt-4 px-6 py-3 flex items-center justify-between sticky top-4 z-50"
    >
      <Link to="/" className="flex items-center gap-3 group">
        <span className="text-2xl">🌿</span>
        <div>
          <h1 className="font-heading text-lg text-aether-800 group-hover:text-aether-700 transition-colors leading-tight">
            Calculadora HC
          </h1>
      ...
      {isAuthenticated && (
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={logout}
            className="flex items-center gap-1.5 text-xs text-surface-500 hover:text-red-600 transition-colors px-2 py-1.5 rounded-full hover:bg-red-500/10"
            title="Cerrar sesión"
          >
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full transition-all ${
        active
          ? "bg-aether-500/15 text-aether-700"
